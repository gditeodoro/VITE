...
class VisualizationRF:
    ...
    def plot_model(self):
        ...
        results = {
            'tree_num': [],
            **{f"{f}": [] for f in list(range(features))},
            **{f"{f}_depth": [] for f in list(range(features))}
        }

        tree_dict = {}  # inizialization of the dictionary

        for i, estimator in enumerate(self.model.estimators_):

            tree_dict[f"tree_{i}"] = {}
            results['tree_num'].append(i)
            for f in list(range(len(self.list_features))):
                results[str(f)].append(0)
                results[str(f) + '_depth'].append(
                    [])

            for j in range(len(estimator.tree_.children_left)):
                node, depth, feat = self.getDepth(estimator.tree_, j)
                tree_dict['tree_' + str(i)][node] = [depth, feat, estimator.tree_.threshold[node]]
                if feat != -2:
                    results[str(feat)][i] += 1
                    results[str(feat) + '_depth'][i].append(depth)
